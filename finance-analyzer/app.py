...
class FinancialAnalyzer:
    ...
    def categorize_transaction(self, row):
        ...
        categories = {
            ...
            'food': ['grocery', 'restaurant', 'deli', 'starbucks'],
            'transportation': ['uber', 'lyft', 'gas', 'metro'],
            'entertainment': ['netflix', 'concert', 'cinema'],
            'utilities': ['electricity', 'water', 'internet'],
            'shopping': ['clothing', 'electronics', 'home goods'],
            'health and wellness': ['pharmacy', 'gym', 'doctor visit']
        }
        ...
        # AI-Based Categorization if Rule-Based Matching Fails
        prompt = f"""Categorize transaction: "{row['description']}" (${row['amount']})
        Options: housing, food, transportation, utilities, entertainment, investment, health and wellness, income, insurance, other
        ...
        try:
            category = response_json['choices'][0].get('text', '').strip().lower()
            return category if category in categories.keys() else category
        except Exception as e:
            st.error(f"Error parsing AI categorization response: {e}")
            return "other"
